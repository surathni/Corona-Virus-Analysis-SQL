...
SELECT MAX(Date) AS Earliest_Start_Date, MIN(Date) AS Latest_End_Date
FROM [Corona Virus Dataset];
...
WITH MonthlyData AS (
    SELECT 
        ...
        Deaths,
        Recovered,
        ROW_NUMBER() OVER (PARTITION BY YEAR(Date), MONTH(Date), Confirmed ORDER BY COUNT(*) DESC) AS Confirmed_Rank,
        ROW_NUMBER() OVER (PARTITION BY YEAR(Date), MONTH(Date), Deaths ORDER BY COUNT(*) DESC) AS Deaths_Rank,
        ROW_NUMBER() OVER (PARTITION BY YEAR(Date), MONTH(Date), Recovered ORDER BY COUNT(*) DESC) AS Recovered_Rank
    FROM [Corona Virus Dataset]
